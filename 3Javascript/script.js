document.addEventListener("DOMContentLoaded", () => {
    ...
    if (courseSearchInput && courseSearchResults) {
        const updateResults = () => {
            ...
            if (!query) return courseSearchResults.style.display = "none";
            ...
            courseSearchResults.style.display = courseSearchResults.children.length ? "block" : "none";
        };

        courseSearchInput.addEventListener("input", updateResults);
        courseSearchBtn.addEventListener("click", () => openCourseModal(courseSearchInput.value.trim()));
        courseSearchInput.addEventListener("keydown", e => e.key === "Enter" && openCourseModal(courseSearchInput.value.trim()));
    ...
    if (scrollLeft && scrollRight && courseBubbles) {
        scrollLeft.addEventListener('click', () => courseBubbles.scrollBy({ left: -350, behavior: 'smooth' }));
